...
function descriptor(slot, data) {
  ...
  return {
    ...
    realm: String(summary.realm || (data && data.realm) || 'runa'),
    level: Math.max(1, Math.floor(Number(summary.level) || 1)),
    pvp: {
      wins: Math.max(0, Math.floor(Number(summary.pvp && summary.pvp.wins) || 0)),
      losses: Math.max(0, Math.floor(Number(summary.pvp && summary.pvp.losses) || 0))
    },
