...
function makeNoxKingdomRows() {
  ...
  const grid = Array.from({ length: height }, (_, y) =>
    Array.from({ length: width }, (_, x) => {
      if ((x * 11 + y * 17) % 43 === 0) return '*'
      return (x * 13 + y * 23) % 31 === 0 ? ',' : '%'
    })
  )
  const set = (x, y, ch) => {
    if (x < 0 || y < 0 || x >= width || y >= height) return
  ...
  const building = (x, y, w, h, label, door, facing = 'south') => {
    fill(x, y, w, h, ';')
    border(x, y, w, h)
    for (let col = x + 2; col < x + w - 1; col += 5) set(col, y - 1, '^')
    for (let row = y + 5; row < y + h - 4; row += 6) {
      write(x, row, '|#')
      write(x + w - 2, row, '#|')
    }
  ...
  const openGarden = (x, y, w, h, label) => {
    fill(x, y, w, h, ',')
    border(x, y, w, h)
    write(x + 3, y + 1, label)
    for (let row = y + 4; row < y + h - 3; row += 5) {
      for (let col = x + 5 + ((row - y) % 3); col < x + w - 4; col += 9) {
        write(col - 1, row, '***')
        set(col, row + 1, '|')
      }
    }
    // Four openings turn the garden into a district, not a decorative prison.
    fill(x + Math.floor(w / 2) - 1, y, 3, 2, '.')
    fill(x + Math.floor(w / 2) - 1, y + h - 2, 3, 2, '.')
    fill(x, y + Math.floor(h / 2) - 1, 2, 3, '.')
    fill(x + w - 2, y + Math.floor(h / 2) - 1, 2, 3, '.')
  }
  ...
  // Volcanic pinnacles break up the cave floor and frame the capital.
  for (const [x, y] of [
    [3, 3],
    [25, 3],
    [151, 3],
  ...
  // An explorable, vertically layered palace dominates the northern cavern.
  fill(49, 2, 82, 32, ';')
  border(49, 2, 82, 32)
  for (let x = 51; x < 130; x += 5) set(x, 1, '^')
  for (const x of [53, 63, 117, 127]) {
    write(x, 4, ' /\\ ')
    write(x, 5, '/##\\')
    write(x, 6, '|[]|')
    write(x, 7, '|##|')
  }
  write(73, 4, 'palacio del eclipse')
  write(78, 7, 'corona de nox')
  for (const x of [58, 70, 109, 121]) {
    write(x, 11, '/\\')
    write(x, 12, '||')
    write(x, 13, '||')
    write(x, 14, '\\/')
  ...
  write(75, 18, '/---------------------------\\')
  write(75, 19, '|      corte de sombras      |')
  write(75, 20, '|    /\\      /\\      /\\    |')
  write(75, 21, '|   /##\\    /^^\\    /##\\   |')
  write(75, 22, '|   |##|    |<>|    |##|   |')
  write(75, 23, '|___|##|____|##|____|##|___|')
  write(83, 26, '/-----------\\')
  write(83, 27, '| trono lunar |')
  write(83, 28, '|    /^^\\    |')
  write(83, 29, '|____|##|____|')
  fill(88, 30, 5, 5, '.')

  const potion = building(5, 9, 39, 25, 'alquimia micelial', 'P')
  const temple = building(136, 9, 39, 25, 'santuario lunar', 'I')
  const weapons = building(5, 69, 43, 27, 'forja de hoja umbria', 'A', 'north')
  const armor = building(132, 69, 43, 27, 'armeria de obsidiana', 'D', 'north')
  const home = building(68, 71, 44, 25, 'casa del linaje', 'C', 'north')
  ...
  // Living light and trade galleries fill what used to be empty gravel.
  openGarden(22, 37, 38, 25, '')
  write(30, 38, 'jardin luminiscente')
  fill(29, 43, 7, 3, '~')
  fill(32, 43, 1, 3, '.')
  fill(46, 54, 8, 3, '~')
  fill(49, 54, 1, 3, '.')
  ...
  fill(120, 37, 38, 25, ';')
  border(120, 37, 38, 25)
  write(129, 38, 'mercado velado')
  for (const x of [124, 135, 145]) {
    write(x, 42, '/-----\\')
    write(x, 43, '| * * |')
    write(x, 44, '|_____|')
    write(x, 55, '/-----\\')
  ...
  // The civic court links every district around a crescent pool and obelisk.
  fill(64, 37, 52, 30, ';')
  write(65, 38, 'corte del eclipse')
  write(68, 42, '  /~~~~~~~~\\  ')
  write(68, 43, ' /~~~~~~~~~~\\ ')
  write(68, 44, ' \\~~~~..~~~~/ ')
  write(68, 45, '  \\________/  ')
  write(99, 43, '    /\\    ')
  write(99, 44, '   /##\\   ')
  write(99, 45, '   |<>|   ')
  write(99, 46, ' __|##|__ ')
  ...
  write(98, 57, ' /##\\/##\\ ')
  write(98, 58, ' | luna  | ')
  write(98, 59, '_|______|_')
  // Repaint the two public avenues last so every service remains connected.
  fill(1, 48, width - 2, 5, '.')
  fill(88, 30, 5, 41, '.')
  fill(20, 62, 140, 4, '.')
  fill(potion.x - 1, potion.y + 1, 3, 48 - potion.y - 1, '.')
