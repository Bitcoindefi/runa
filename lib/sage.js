...
function buildThresholds(content, maxhp) {
  ...
  // "Cerca" y "lejos" son parte del idioma publicado. Las nuevas armas no
  // deben cambiar silenciosamente scripts existentes: espada y ballesta siguen
  // siendo sus referencias canonicas cuando estan en el contenido.
  const short =
    content.items.sword || weapons.reduce((a, b) => (b.reach < a.reach ? b : a), weapons[0])
  const long =
    content.items.crossbow || weapons.reduce((a, b) => (b.reach > a.reach ? b : a), weapons[0])
