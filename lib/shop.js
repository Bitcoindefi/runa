 ...
 * What a foe is worth.
 *
 * A hand tuned payout in the foe's own `drop` table wins. Without one the
 * numbers are derived from the foe's stats, so a foe shipped over the air pays
 * out the moment it lands with no second table to keep in sync.
 *
 * This read `def.drops` and content.js has always written `drop`, so the
 * override never fired once: every foe in the game fell through to the derived
 * numbers. That is not a rounding difference. Derived, a mosquito and a spectre
 * are both worth 12 gold and 18 xp, while the table says a mosquito is worth 2
 * to 5 gold and 3 xp and a golem 14 to 22 and 18. The table is the entire
 * relationship between how hard a thing is and what killing it is worth, and it
 * was switched off by a plural.
 *
 * @param {string} foeId
...
function reward(foeId) {
  ...
  const table = def.drop
  if (table) {
    const gold = table.gold
    return {
      xp: table.xp | 0,
      // Gold is a range so that two mosquitoes are not worth the same. With no
      // roll to hand this is the middle of it: whoever calls without rolling
      // wants one honest number, not a lucky or an unlucky one.
      gold: Array.isArray(gold) ? Math.round((gold[0] + gold[1]) / 2) : gold | 0
    }
  }
...
class Player {
   ...
   * @param {{xp: number, gold: number}} [earned] - what the field already rolled
  ...
  settle(world, earned) {
    ...
    // The field rolls the gold when the foe falls, with its own seeded rng, and
    // hands the number over. Taking it rather than rolling again is what keeps
    // the line the player reads and the gold the player gets the same number by
    // construction: there is only one roll, so the two cannot drift apart.
    const drop = earned && earned.gold !== undefined ? earned : reward(world.foeDef.id)
