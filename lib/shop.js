 ...
 * moment it lands with no second table to keep in sync. A foe that wants a
 * hand-tuned payout carries `drops: { xp, gold }` and that wins.
 *
 * @param {string} foeId
...
function reward(foeId) {
  ...
  if (def.drops) return { xp: def.drops.xp | 0, gold: def.drops.gold | 0 }
