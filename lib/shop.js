...
const EQUIPMENT_SLOTS = Object.freeze(['left_hand', 'right_hand', 'chest', 'head', 'boots'])
...
const shops = {
  ...
  weapons: {
    ...
    sells: [
      { id: 'dagger', price: 15 },
      { id: 'sword', price: 25 },
      { id: 'spear', price: 45, minLevel: 2 },
      { id: 'crossbow', price: 60, minLevel: 2 },
      { id: 'warhammer', price: 85, minLevel: 4 },
      { id: 'longbow', price: 100, minLevel: 5 }
    ]
  },
  ...
  armor: {
    id: 'armor',
    name: 'armeria',
    keeper: 'la maestra armera',
    glyph: '0',
    about: 'escudos y proteccion para pecho, cabeza y pies',
    sells: [
      { id: 'leather', price: 15 },
      { id: 'leather_cap', price: 20 },
      { id: 'shield', price: 30 },
      { id: 'boots', price: 45, minLevel: 2 },
      { id: 'chainmail', price: 65, minLevel: 3 },
      { id: 'iron_helmet', price: 75, minLevel: 3 },
      { id: 'plate', price: 110, minLevel: 5 }
    ]
  }
}
...
class Player {
  ...
  constructor(opts = {}) {
    ...
    this.storage = new Set(opts.storage || [])
    for (const id of this.items) this.storage.delete(id)
    this.pvp = {
      wins: Math.max(0, Math.floor(Number(opts.pvp && opts.pvp.wins) || 0)),
      losses: Math.max(0, Math.floor(Number(opts.pvp && opts.pvp.losses) || 0))
    }
    this.equipped = emptyEquipment()
    const savedEquipment = opts.equipped || {}
    const legacy = { left_hand: savedEquipment.left, right_hand: savedEquipment.right }
    for (const slot of EQUIPMENT_SLOTS) {
      const id = savedEquipment[slot] || legacy[slot]
      const item = CONTENT.items[id]
      if (item && item.slot === slot && this.items.has(id)) this.equipped[slot] = id
    }
  ...
  /** True whether the item is carried or safely stored at home. */
  hasItem(id) {
    return this.items.has(id) || this.storage.has(id)
  }
  ...
  /** Equip one carried item in the exact body slot declared by content. */
  equip(id) {
    const item = CONTENT.items[id]
    if (!item) return { ok: false, reason: `no existe "${id}"` }
    if (!this.owns(id)) return { ok: false, reason: `no tenes ${item.name}` }
    const slot = item.slot
    if (!EQUIPMENT_SLOTS.includes(slot)) {
      return { ok: false, reason: `${item.name} no se puede equipar` }
    }
    this.equipped[slot] = id
    return { ok: true, reason: '', id, slot, item }
  }

  /** Remove an item from its slot without removing it from the inventory. */
  unequip(idOrSlot) {
    const item = CONTENT.items[idOrSlot]
    const aliases = { left: 'left_hand', right: 'right_hand' }
    const requested = aliases[idOrSlot] || idOrSlot
    const slot = EQUIPMENT_SLOTS.includes(requested) ? requested : item && item.slot
    if (!slot || !this.equipped[slot]) return { ok: false, reason: 'eso no esta equipado' }
    const id = this.equipped[slot]
    if (item && id !== idOrSlot) return { ok: false, reason: `${item.name} no esta equipado` }
    this.equipped[slot] = null
    return { ok: true, reason: '', id, slot, item: CONTENT.items[id] }
  }
  ...
  /** Move carried equipment into the persistent chest at home. */
  deposit(id) {
    const item = CONTENT.items[id]
    if (!item) return { ok: false, reason: `no existe "${id}"` }
    if (!this.owns(id)) return { ok: false, reason: `no llevas ${item.name}` }
    if (this.isEquipped(id)) this.unequip(id)
    this.items.delete(id)
    this.storage.add(id)
    return { ok: true, reason: '', id, item }
  }

  /** Return equipment from the home chest to the carried inventory. */
  withdraw(id) {
    const item = CONTENT.items[id]
    if (!item) return { ok: false, reason: `no existe "${id}"` }
    if (!this.storage.has(id)) return { ok: false, reason: `${item.name} no esta depositado` }
  ...
  recordDuel(won) {
    if (won) this.pvp.wins++
    else this.pvp.losses++
    return { ...this.pvp }
  }
  ...
  canBuy(goodId, shopId) {
    ...
    if (!def.stack && this.hasItem(goodId)) {
      return no(
        this.stored(goodId) ? `${def.name} esta en tu deposito` : `ya tenes ${def.name}`,
        entry.price,
        shop.id
      )
    }
  ...
  sell(goodId, shopId) {
    ...
    if (stack) this.potions--
      ...
      if (this.isEquipped(goodId)) this.unequip(goodId)
      this.items.delete(goodId)
    }
    this.gold += paid
  ...
  outfit(world) {
    ...
    world.held.left = CONTENT.items[this.equipped.left_hand] || null
    world.held.right = CONTENT.items[this.equipped.right_hand] || null
    world.held.chest = CONTENT.items[this.equipped.chest] || null
    world.held.head = CONTENT.items[this.equipped.head] || null
    world.held.boots = CONTENT.items[this.equipped.boots] || null
    return world
  }
...
const MIGRATIONS = [
  ...
  },
  {
    ...
    to: 2,
    up(d) {
      return { ...d, version: 2, equipped: { left: null, right: null } }
    }
  },
  {
    from: 2,
    to: 3,
    up(d) {
      return { ...d, version: 3, pvp: { wins: 0, losses: 0 } }
    }
  },
  {
    from: 3,
    ...
    up(d) {
      ...
      if (CONTENT.items[old.left] && CONTENT.items[old.left].slot === 'left_hand') {
        equipped.left_hand = old.left
      }
      if (CONTENT.items[old.right]) {
        const slot = CONTENT.items[old.right].slot
        if (EQUIPMENT_SLOTS.includes(slot)) equipped[slot] = old.right
      }
      return { ...d, version: 4, equipped, storage: [] }
    }
  }
]
...
function browse(shopId, player) {
  ...
  const lines = shop.sells.map((entry) => {
    ...
    return {
      ...
      ok: deal.ok,
      reason: deal.reason,
      owned: player ? player.owns(entry.id) : false,
      stored: player ? player.stored(entry.id) : false,
      equipped: player ? player.isEquipped(entry.id) : false
    }
  })
