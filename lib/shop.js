...
class Player {
  ...
  constructor(opts = {}) {
    ...
    for (const slot of ['left', 'right']) {
      const id = opts.equipped && opts.equipped[slot]
      const item = CONTENT.items[id]
      if (item && item.hand === slot && this.items.has(id)) this.equipped[slot] = id
    }
  ...
  /** Equip one owned item in the hand/armour slot declared by content. */
  equip(id) {
    const item = CONTENT.items[id]
    if (!item) return { ok: false, reason: `no existe "${id}"` }
    if (!this.owns(id)) return { ok: false, reason: `no tenes ${item.name}` }
    const slot = item.hand === 'right' ? 'right' : 'left'
  ...
  unequip(idOrSlot) {
    ...
    const slot = idOrSlot === 'left' || idOrSlot === 'right' ? idOrSlot : item && item.hand
  ...
  isEquipped(id) {
    return this.equipped.left === id || this.equipped.right === id
  }
  ...
  outfit(world) {
    ...
    world.held.left = CONTENT.items[this.equipped.left] || null
    world.held.right = CONTENT.items[this.equipped.right] || null
    return world
  }
