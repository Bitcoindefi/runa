...
const { ownedOnly } = require('./shop.js')
...
/** Western wall gate: its `<` cell sits exactly on the RUNA edge. */
const RUNA_GATE_ART = [
  '|\\        /\\      /\\',
...
/** Eastern dark gate: the final `N` cell touches the opposite map edge. */
const NOX_GATE_ART = [
  '        /\\      /\\        /|',
...
const KINGDOM_GATE_CLEARANCE = { x: 30, y: 10 }

function insideKingdomGateClearing(point, x, y) {
  return (
    Math.abs(x - point.x) < KINGDOM_GATE_CLEARANCE.x &&
    Math.abs(y - point.y) < KINGDOM_GATE_CLEARANCE.y
  )
}
...
const FIELD_GUIDES = [
  {
    id: 'eira',
    name: 'Eira',
    role: 'guardabosques del camino real',
    x: 50,
    y: 36,
    line: 'segui las marcas del camino: al norte esta el portal, al sur la cripta y hay empalizadas barbaras al este'
  }
]

const RANGER_POST_ART = [
...
const CROSSROADS_ART = ['runa <---+---> nox', '         |', '   portal|cripta']

const BARBARIAN_SETTLEMENTS = Object.freeze([
  {
    id: 'colmillo-rojo',
    name: 'colmillo rojo',
    x: 104,
...
const BARBARIAN_SETTLEMENT_CLEARANCE = 18

function barbarianSettlementArt(name) {
  const label = String(name || 'campamento')
    .slice(0, 21)
    .padEnd(21)
  return [
    '       /\\             /\\',
...
function segmentDistance(x, y, ax, ay, bx, by) {
  ...
  const amount = length ? clamp(((x - ax) * vx + (y - ay) * vy) / length, 0, 1) : 0
  return Math.hypot(x - (ax + vx * amount), (y - (ay + vy * amount)) * Y_SCALE)
}

/** Authored landscape bands laid over deterministic noise. */
function fieldGroundAt(field, wx, wy) {
  const cross = { x: 66, y: field.gate.y }
  const roadDistance = Math.min(
    segmentDistance(wx, wy, field.gate.x, field.gate.y, cross.x, cross.y),
    segmentDistance(wx, wy, cross.x, cross.y, field.noxGate.x, field.noxGate.y),
    segmentDistance(wx, wy, cross.x, cross.y, field.worldBossPortal.x, field.worldBossPortal.y),
    segmentDistance(wx, wy, cross.x, cross.y, field.dungeonEntrance.x, field.dungeonEntrance.y),
    ...BARBARIAN_SETTLEMENTS.map((settlement) =>
      segmentDistance(wx, wy, settlement.x, field.gate.y, settlement.x, settlement.y)
    )
  )
  if (roadDistance <= 1.35) return (wx + wy) % 11 === 0 ? '%' : ':'
  if (roadDistance <= 2.4) return (wx * 3 + wy) % 7 === 0 ? ',' : ' '

  const noise = (wx * 17 + wy * 31 + wx * wy * 3) % 97
  const zone = zoneAt(field.gate, wx, wy)
  const ground = CONTENT.zones[zone].ground

  // Irregular enclosed fields and ridge-and-furrow remain strongest near RUNA.
  if (wx > 18 && wx < 75 && wy > 5 && wy < field.height - 5) {
    const boundary = [8, 17, 33, 41].some((line) => Math.abs(wy - line) === 0)
    if (boundary && wx % 6 !== 0) return '"'
    if ((wx + Math.floor(wy / 3)) % 9 === 0) return ','
    if ((wx * 5 + wy) % 29 === 0) return '*'
    return noise % 11 === 0 ? ground : ' '
  }

  // Small wooded clearings break the middle distance into memorable regions.
  for (const grove of [
    { x: 82, y: 9, rx: 15, ry: 5 },
    { x: 82, y: 36, rx: 13, ry: 5 }
  ]) {
    const dx = (wx - grove.x) / grove.rx
    const dy = (wy - grove.y) / grove.ry
    if (dx * dx + dy * dy < 1) {
      if (noise % 11 === 0) return '*'
      return noise % 7 === 0 ? '"' : ' '
    }
  }

  // The eastern yermo reads as worn, stony ground before the NOX frontier.
  if (wx > 96) {
    if (noise === 0 || noise === 37 || noise === 71) return 'o'
    if (noise % 13 === 0) return ','
    return noise % 29 === 0 ? '%' : ' '
  }

  if (noise === 0 || noise === 37) return 'o'
  if (zone === 0 && (noise === 11 || noise === 53)) return '*'
  if (zone === 0 && noise % 23 === 0) return '"'
  if (zone > 0 && noise % 19 === 0) return ','
  return noise % 17 === 0 ? ground : ' '
}
...
class Field {
  ...
  constructor(opts = {}) {
    ...
    /** RUNA and NOX occupy opposite edges of the meadow. */
    this.gate = { x: 0, y: Math.floor(this.height / 2) }
    this.noxGate = { x: this.width - 1, y: Math.floor(this.height / 2) + 2 }
    /** The crypt occupies the southern interior, well before NOX's border. */
    this.dungeonEntrance = { x: Math.floor(this.width * 0.55), y: this.height - 5 }
    /** The Colossus portal stands in the north-central yermo, apart from NOX. */
    this.worldBossPortal = { x: Math.floor(this.width * 0.7), y: 12 }
    this.guides = FIELD_GUIDES.map((guide) => ({ ...guide }))
    const clearedSettlements = new Set(
      Array.isArray(opts.clearedSettlements) ? opts.clearedSettlements.map(String) : []
    )
    this.settlements = BARBARIAN_SETTLEMENTS.filter(
      (settlement) => !clearedSettlements.has(settlement.id)
    ).map((settlement) => ({ ...settlement, roster: settlement.roster.slice() }))
    this.clearedSettlements = clearedSettlements
    ...
    this.nodes = []
    this.scriptErrors = []
    // Separate from the excursion snapshot: scripts may only wield inventory.
    this.owner = opts.player && typeof opts.player.owns === 'function' ? opts.player : null
  ...
  findSpot(z) {
    for (let tries = 0; tries < 400; tries++) {
      ...
      if (insideKingdomGateClearing(this.gate, x, y)) continue
      if (insideKingdomGateClearing(this.noxGate, x, y)) continue
      if (this.boss && this.boss.occupies(x, y)) continue
      if (this.guideAt(x, y, 3)) continue
      if (this.settlementAt(x, y, BARBARIAN_SETTLEMENT_CLEARANCE)) continue
  ...
  guideAt(x, y, range = 0) {
    return (
      this.guides.find(
        (guide) => Math.max(Math.abs(guide.x - x), Math.abs(guide.y - y)) <= range
      ) || null
    )
  }

  settlementAt(x, y, range = 0) {
    return (
      this.settlements.find(
        (settlement) => Math.max(Math.abs(settlement.x - x), Math.abs(settlement.y - y)) <= range
      ) || null
    )
  }

  settlementStructureAt(x, y) {
    for (const settlement of this.settlements) {
      const art = barbarianSettlementArt(settlement.name)
      const entryRow = art.findIndex((line) => line.includes('J'))
      const entryCol = art[entryRow].indexOf('J')
      const sy = y - (settlement.y - entryRow)
      if (sy < 0 || sy >= art.length) continue
      const sx = x - (settlement.x - entryCol)
      if (sx < 0 || sx >= art[sy].length || art[sy][sx] === ' ') continue
      return { settlement, glyph: art[sy][sx] }
    }
    return null
  }
  ...
  clearSettlement(id) {
    ...
    this.settlements = this.settlements.filter((settlement) => settlement.id !== campId)
    if (before !== this.settlements.length) this.clearedSettlements.add(campId)
    return before !== this.settlements.length
  }

  interact() {
    const guide = this.guideAt(this.player.x, this.player.y, 2)
    return guide
      ? [{ type: 'field-guide', text: `${guide.name}, ${guide.role}: ${guide.line}` }]
      : [{ type: 'field-empty', text: 'no hay nadie cerca con quien hablar' }]
  }
  ...
  walk(dx, dy) {
    ...
    const guide = this.guideAt(nx, ny)
    if (guide) {
      return [{ type: 'field-guide', text: `${guide.name}, ${guide.role}. pulsa e para hablar` }]
    }

    const settlement = this.settlementAt(nx, ny)
    if (settlement) {
      this.player.x = nx
      this.player.y = ny
      return [
        {
          type: 'barbarian-enter',
          campId: settlement.id,
          text: `entras al asentamiento ${settlement.name}`
        }
      ]
    }

    const structure = this.settlementStructureAt(nx, ny)
    if (structure) {
      return [{ type: 'barbarian-wall', text: 'la empalizada bloquea el paso; busca la puerta J' }]
    }
    ...
    if (nx === this.noxGate.x && ny === this.noxGate.y) return [{ type: 'nox' }]
  ...
  wander(events) {
    for (const f of this.foes) {
      ...
      if (insideKingdomGateClearing(this.gate, nx, ny)) continue
      if (insideKingdomGateClearing(this.noxGate, nx, ny)) continue
      if (this.boss && this.boss.occupies(nx, ny)) continue
      if (this.guideAt(nx, ny, 3)) continue
      if (this.settlementAt(nx, ny, BARBARIAN_SETTLEMENT_CLEARANCE)) continue
  ...
  stepFight(events) {
    ...
    const gated = this.owner
      ? ownedOnly(this.owner, out.actions)
      : { actions: out.actions, problems: [] }
    const problems = [...gated.problems, ...w.readIntent(gated.actions)]
  ...
  snapshot() {
    ...
    return {
      ...
      clearedSettlements: [...this.clearedSettlements],
      mode: 'field',
      player: { ...this.player },
      zone: this.zone.name,
  ...
  render(cols, rows, includeActors = true) {
    ...
    for (let y = 0; y < vh; y++) {
      ...
      for (let x = 0; x < vw; x++) {
        const wx = ox + x
        const wy = oy + y
        row[x] = fieldGroundAt(this, wx, wy)
      }
      grid.push(row)
    }
    ...
    const paintAnchoredArt = (point, art, anchorRow, anchorCol, transparentSpaces = true) => {
      for (let sy = 0; sy < art.length; sy++) {
        for (let sx = 0; sx < art[sy].length; sx++) {
          if (transparentSpaces && art[sy][sx] === ' ') continue
          put(point.x + sx - anchorCol, point.y + sy - anchorRow, art[sy][sx])
        }
      }
    }
    paintAnchoredArt(this.gate, RUNA_GATE_ART, 7, 0, false)
    paintAnchoredArt(this.noxGate, NOX_GATE_ART, 7, NOX_GATE_ART[7].lastIndexOf('N'), false)
    paintAnchoredArt({ x: 66, y: this.gate.y }, CROSSROADS_ART, 0, 9, false)
    paintAnchoredArt({ x: 50, y: 33 }, RANGER_POST_ART, 4, 13, false)
    for (const settlement of this.settlements) {
      const art = barbarianSettlementArt(settlement.name)
      const entryRow = art.findIndex((line) => line.includes('J'))
      const entryCol = art[entryRow].indexOf('J')
      paintAnchoredArt(settlement, art, entryRow, entryCol, false)
    }
