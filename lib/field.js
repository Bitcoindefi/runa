...
class Field {
  ...
  constructor(opts = {}) {
    ...
    /**
     * The persistent sheet, kept only to answer "does this player own that".
     *
     * Deliberately NOT merged with `this.player` below, which is the excursion's
     * own throwaway bookkeeping. Two different lifetimes: one dies when you walk
     * home, the other is your character. Merging them is a real refactor and
     * this is not it.
     */
    this.owner = opts.player || null
  ...
  stepFight(events) {
    ...
    // Ownership is enforced here, between the rules and the world, exactly
    // where shop.js says it should be.
    //
    // It was not enforced anywhere. `ownedOnly` was written, exported,
    // documented with this very call site in its comment, and never called by
    // anything, so `equip crossbow` handed you a crossbow you had never bought.
    // Every shop in the game was decoration: there was no reason to earn gold
    // because the gear was free.
    //
    // The refusals are worth more than the block. A rule that silently does
    // nothing is the worst thing this language can do to somebody, so the
    // reason travels back and gets said out loud: "no tenes ballesta", while
    // the fight carries on with whatever you do have.
    const gated = this.owner
      ? ownedOnly(this.owner, out.actions)
      : { actions: out.actions, problems: [] }
    const problems = w.readIntent(gated.actions)

    // Once per fight per reason, not once per tick.
    //
    // The rules are re-read fifteen times a second, so a rule that reaches for
    // a crossbow you do not own produces a refusal fifteen times a second, and
    // the first version of this filled the log with the same six words over and
    // over until nothing else was readable. ownedOnly's own comment warned
    // about exactly this and I wrote it anyway. The set lives on the fight, so
    // it clears itself when the fight does and the next monster gets told
    // again.
    if (!c.refused) c.refused = new Set()
    for (const why of gated.problems) {
      if (c.refused.has(why)) continue
      c.refused.add(why)
      this.say(why)
    }
