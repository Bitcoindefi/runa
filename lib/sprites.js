...
function makeMovingHeroSprite(options = {}) {
  ...
  const initial =
    String(options.initial || 'T')
      ...
      .slice(0, 1)
      .toUpperCase() || 'T'
  const weapon = ids.has('crossbow') ? 'crossbow' : ids.has('sword') ? 'sword' : null
  const shield = ids.has('shield')
  const boots = ids.has('boots')
  const walking = frame % 2 === 1

  const head = weapon === 'sword' ? (walking ? '| O' : '/ O') : '  O'
  let torso = walking ? ` \\${initial}-` : ` /${initial}\\`
  if (weapon === 'sword') torso = walking ? `/|${initial}-` : `/|${initial}\\`
  else if (weapon === 'crossbow') torso = walking ? `}-${initial}-` : `}-${initial}\\`
  if (shield) torso += ' [#]'
