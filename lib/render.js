    ...
    if (page === 'main') {
      ...
      for (let index = 0; index < slots.length; index++) {
        ...
        if (slot.empty) label += 'VACIA'
          ...
          label += `${ascii(slot.name || 'viajero')} | nv ${slot.level || 1} | ${ascii(slot.place || 'ciudad')}`
        }
...
function mapPane(map, w, h, opts = {}) {
  ...
  for (const a of (map && map.actors) || []) {
    ...
    if (Math.max(Math.abs(ax - hx), Math.abs(ay - hy)) <= 2) {
    ...
    for (let sy = 0; sy < actorSprite.length; sy++) {
      ...
      for (let sx = first; sx <= last; sx++) {
        // Every space is transparent. Painting an interior space would erase
...
function fieldPane(field, w, h) {
  ...
  if (boss && !boss.defeated) {
    const bossArt = WORLD_BOSS.fieldSprite.frames[boss.frame] || WORLD_BOSS.fieldSprite.frames.idle
