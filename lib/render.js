  ...
  if (!slots) {
    ...
    if (page === 'main') {
      ...
      body.push(
        '',
        centre(
          style().faint(true).render('ARRIBA/ABAJO | ENTER / ESPACIO aceptar | N nueva | Q salir')
        )
      )
    } else {
...
function mapPane(map, w, h, opts = {}) {
  ...
  for (const a of (map && map.actors) || []) {
    ...
    const isOverlapping =
      heroLeft <= defaultActorRight && heroRight >= defaultActorLeft && Math.abs(ay - hy) <= 2
  ...
  for (let r = 0; r < h; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      if (isActor) ch = g
      const cell = expandCell(ascii(ch), cellW, isActor)
...
function inventoryPane(model, w, h) {
  ...
  const lines = [
    row(
      ...
      style()
        .bold(true)
        .render(home ? 'DEPOSITO DEL HOGAR' : 'INVENTARIO'),
  ...
  lines.push(
    row(w, 'izq ' + itemLabel(equipped.left_hand), 'der ' + itemLabel(equipped.right_hand))
  )
  lines.push(row(w, 'pecho ' + itemLabel(equipped.chest), 'casco ' + itemLabel(equipped.head)))
  lines.push(row(w, 'botas ' + itemLabel(equipped.boots)))
  lines.push('-'.repeat(w))
  ...
  for (let index = 0; index < items.length; index++) {
    ...
    const tag = item.equipped ? 'equipado' : slot
    lines.push(
      row(
        w,
        selected
          ? style()
              .bold(true)
              .render(marker + itemLabel(item))
          : marker + itemLabel(item),
        tag
      )
    )
  }
  ...
  if (selected && selected.about) {
    lines.push('-'.repeat(w))
    for (const line of wrap(ascii(selected.about), w)) lines.push(style().faint(true).render(line))
  }
  return style().width(w).height(h).render(lines.join('\n'))
}
