...
function mapColor(ch) {
  if (ch === ',' || ch === 't') return 'green'
  if (ch === '~') return 'blue'
  if (ch === '*' || ch === '^') return 'magenta'
  if (ch === 'O') return 'cyan'
  if (ch === 'o' || '#+-|;'.includes(ch)) return 'gray'
 ...
 * @returns {string}
 */
function titleScreen(w, h, status) {
  ...
  const body = [
    ...
    '',
    centre(style().reverse(true).render(' cualquier tecla para empezar '))
  ]
...
function mapPane(map, w, h, opts = {}) {
  ...
  for (let r = 0; r < h; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      const isHero = isActor && x === hx && y === hy
      line += paint(cell, isHero ? COLOR.hero : isActor ? COLOR.foe : mapColor(ch))
    }
