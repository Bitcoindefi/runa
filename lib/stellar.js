...
class Chain {
  ...
  /** Read the current account sequence without ever requesting signing keys. */
  async account(address) {
    if (!isPublicAddress(address)) throw new Error('la cuenta Stellar no es valida')
    const r = await post(this.net.horizon, '/accounts/' + address, null)
    if (r.status !== 200 || !r.body || !r.body.sequence) {
      throw new Error(
        r.status === 404 ? 'la cuenta Stellar no existe' : 'Horizon no devolvio la cuenta'
      )
    }
    return { address, sequence: String(r.body.sequence) }
  }

  /**
   * Build and simulate one Soroban invocation, returning XDR ready for an
   * injected wallet. The secret key never crosses this boundary.
   */
  async prepareInvocation({ source, contractId, method, args = [], timeout = 180 }) {
    if (!base) throw new Error('la criptografia no cargo en este build')
    if (!base.StrKey.isValidContract(contractId)) {
      throw new Error('el contrato Soroban no es valido')
    }
    const account = await this.account(source)
    const operation = new base.Contract(contractId).call(method, ...args)
    const raw = new base.TransactionBuilder(new base.Account(source, account.sequence), {
      fee: BASE_FEE,
    ...
    const simulation = await this.rpc('simulateTransaction', {
      ...
      resourceConfig: { instructionLeeway: 50000 }
    })
    if (!simulation || simulation.error || !simulation.transactionData) {
      throw new Error((simulation && simulation.error) || 'la simulacion Soroban fallo')
    }
    return assembleInvocation(raw, simulation, this.net.passphrase).toXDR()
  }

  /** Submit wallet-signed XDR. A PENDING response is deliberately explicit. */
  sendSigned(xdr) {
    if (typeof xdr !== 'string' || !xdr) throw new Error('el XDR firmado esta vacio')
    return this.rpc('sendTransaction', { transaction: xdr })
  }

  /** Poll an already submitted transaction until it reaches a terminal state. */
  async waitForTransaction(hash, attempts = 20) {
    for (let i = 0; i < attempts; i++) {
      const result = await this.rpc('getTransaction', { hash })
      if (result && result.status !== 'NOT_FOUND') return result
      await new Promise((resolve) => setTimeout(resolve, 500))
    }
    throw new Error('la transaccion sigue pendiente')
  }
...
/** Apply the simulation footprint, auth and resource fee to a raw invocation. */
function assembleInvocation(raw, simulation, passphrase) {
  if (!base) throw new Error('la criptografia no cargo en este build')
  const envelope = raw.toEnvelope()
  const tx = envelope.v1().tx()
  const resourceFee = BigInt(simulation.minResourceFee || 0)
  const totalFee = BigInt(raw.fee) + resourceFee
  if (totalFee > 0xffffffffn) throw new Error('la tarifa Soroban excede el limite')
  tx.fee(Number(totalFee))
  tx.ext(
    new base.xdr.TransactionExt(
      1,
      base.xdr.SorobanTransactionData.fromXDR(simulation.transactionData, 'base64')
    )
  )
  const auth = simulation.results && simulation.results[0] && simulation.results[0].auth
  if (Array.isArray(auth)) {
    tx.operations()[0]
      .body()
      .invokeHostFunctionOp()
      .auth(auth.map((entry) => base.xdr.SorobanAuthorizationEntry.fromXDR(entry, 'base64')))
  }
  return new base.Transaction(envelope, passphrase)
}

function scVal(value, type) {
  if (!base) throw new Error('la criptografia no cargo en este build')
  return base.nativeToScVal(value, type ? { type } : undefined)
}

module.exports = { Chain, TESTNET, assembleInvocation, isPublicAddress, scVal }
