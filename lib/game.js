...
const { BarbarianCamp } = require('./barbarian-camp.js')
...
function barbarianProgress(raw = {}) {
  const cleared = Array.isArray(raw.cleared) ? [...new Set(raw.cleared.map(String))] : []
  const camps = {}
  if (raw.camps && typeof raw.camps === 'object') {
    for (const [id, state] of Object.entries(raw.camps)) {
      camps[String(id)] = state && typeof state === 'object' ? { ...state } : {}
    }
  }
  return { cleared, camps }
}
...
class Runa {
  ...
  constructor(opts = {}) {
    ...
    this.barbarianState = barbarianProgress()
    this.duel = null
    ...
    for (const e of events || []) {
      ...
      switch (e.type) {
        ...
        case 'nox':
          ...
          break
        case 'barbarian-enter': {
          if (!this.field || this.field.mode !== 'field') break
          const settlement = this.field.settlementAt
            ? this.field.settlementAt(this.field.player.x, this.field.player.y)
            : null
          if (!settlement || settlement.id !== e.campId) break
          this.meadowField = this.field
          this.meadowReturn = {
            x: Math.max(1, settlement.x - 2),
            y: settlement.y
          }
          this.field = new BarbarianCamp({
            campId: settlement.id,
            player: this.player,
            script: this.scriptSource,
            seed: this.fieldSeed(),
            state: this.barbarianState.camps[settlement.id]
          })
          this.seen = new Set()
          this.say(`atravesas la empalizada de ${settlement.name}`)
          break
        }
        case 'barbarian-exit': {
          if (!this.field || this.field.mode !== 'barbarian-camp') break
          const camp = this.field.camp
          this.barbarianState.camps[camp.id] = this.field.toJSON()
          const wasCleared = this.barbarianState.cleared.includes(camp.id)
          if (e.cleared && !wasCleared) {
            this.barbarianState.cleared.push(camp.id)
            this.player.gainGold(camp.reward)
          }
          ...
          const restored =
            ...
            new Field({
              ...
              seed: this.fieldSeed(),
              script: this.scriptSource,
              clearedSettlements: this.barbarianState.cleared
            })
          if (e.cleared) restored.clearSettlement(camp.id)
          restored.player.hp = this.player.hp
          restored.player.maxhp = this.player.maxHp
          restored.player.potions = this.player.potions
          restored.player.x = back ? back.x : Math.max(1, camp.x - 2)
          restored.player.y = back ? back.y : camp.y
          this.field = restored
          this.meadowField = null
          this.meadowReturn = null
          this.seen = new Set()
          if (e.cleared) {
            this.say(`asentamiento ${camp.name} eliminado: +${wasCleared ? 0 : camp.reward} oro`)
          } else {
            this.say(`abandonas ${camp.name}; quedan barbaros y la empalizada permanece`)
          }
          break
        }
        case 'dungeon-enter':
  ...
  saveState() {
    ...
    if (this.field) {
      ...
      location =
        field.mode === 'boss'
          ...
          : field.mode === 'barbarian-camp'
            ? {
                kind: 'barbarian-camp',
                campId: field.camp.id,
                x: field.player.x,
                y: field.player.y,
                state: this.field.toJSON(),
                meadowReturn: this.meadowReturn ? { ...this.meadowReturn } : null
    ...
    const barbarianState = barbarianProgress(this.barbarianState)
    if (this.field && this.field.mode === 'barbarian-camp') {
      barbarianState.camps[this.field.camp.id] = this.field.toJSON()
    }
  ...
  loadSlot(slot = this.slotCursor + 1) {
    ...
    try {
      ...
      this.barbarianState = barbarianProgress(saved.barbarianState)
      this.duel = null
      this.duelCombat = null
      this.lastDuelResult = null
      this.duelInvite = null
      ...
      } else if (location.kind === 'barbarian-camp') {
        this.field = new BarbarianCamp({
          campId: location.campId,
          script: this.scriptSource,
          player: this.player,
          seed: this.fieldSeed(),
          state: location.state || this.barbarianState.camps[location.campId],
          x: location.x,
          y: location.y
        })
        this.barbarianState.camps[this.field.camp.id] = this.field.toJSON()
        this.meadowReturn = location.meadowReturn || null
  ...
  returnToCity() {
    ...
    if (this.field.mode === 'barbarian-camp') {
      this.say('debes salir del asentamiento por la puerta marcada con U')
      return false
    }
  ...
  onKey(msg) {
    ...
    if (key.matches(msg, 'f') && this.field) {
      if (this.field.mode === 'dungeon' || this.field.mode === 'barbarian-camp') {
        this.say('acercate a un monstruo para entrar en combate')
    ...
    if (key.matches(msg, 'e', 'enter', 'space')) {
      if (this.field && this.field.mode === 'field' && typeof this.field.interact === 'function') {
        this.drain(this.field.interact())
        return null
      }
      this.enter()
      return null
    ...
    if (this.field) {
      ...
      const area =
        snap.mode === 'dungeon'
          ...
          : snap.mode === 'barbarian-camp'
            ? `campamento ${snap.camp.name}`
            : snap.mode === 'boss'
              ? 'world boss'
              : snap.zone || 'pradera'
      return {
  ...
  view() {
    ...
    if (this.inventoryOpen) {
      ...
      return render.inventoryScreen({
        ...
        equipment,
        items,
        combat: this.duelStats(this.player.snapshot()),
    ...
    if (this.field) {
      ...
      const nearbyGuide = (snap.guides || [])
        .map((guide) => ({
          ...guide,
          distance: Math.round(
            Math.hypot(guide.x - snap.player.x, (guide.y - snap.player.y) * Y_SCALE)
          )
        }))
        .sort((a, b) => a.distance - b.distance)[0]
      const nearbySettlement = (snap.settlements || [])
        .map((settlement) => ({
          ...settlement,
          distance: Math.round(
            Math.hypot(settlement.x - snap.player.x, (settlement.y - snap.player.y) * Y_SCALE)
          )
        }))
        .sort((a, b) => a.distance - b.distance)[0]
      const fieldCaption = snap.combat
        ...
        : snap.mode === 'dungeon'
          ...
          : snap.mode === 'barbarian-camp'
            ? snap.completed
              ? `ASENTAMIENTO ${snap.camp.name.toUpperCase()} LIMPIO | U salir y cobrar ${snap.camp.reward} oro`
              : `ASENTAMIENTO ${snap.camp.name.toUpperCase()} | quedan ${snap.remaining} barbaros | U salir`
            : snap.mode === 'boss' && snap.boss && snap.boss.defeated
              ? 'RUINAS VOLCANICAS | el Coloso ha sido derrotado | O volver'
              : snap.gate &&
                  Math.hypot(
                    snap.gate.x - snap.player.x,
                    (snap.gate.y - snap.player.y) * Y_SCALE
                  ) <= 18
                  ...
                  : snap.dungeonEntrance &&
                    ...
                    : snap.worldBossPortal &&
                      ...
                      : snap.boss && snap.boss.active && !snap.boss.defeated
                        ...
                        : nearbyGuide && nearbyGuide.distance <= 6
                          ? `${nearbyGuide.name}, ${nearbyGuide.role} | E hablar`
                          : nearbySettlement && nearbySettlement.distance <= 18
                            ? `EMPALIZADA ${nearbySettlement.name.toUpperCase()} | J entrar | botin ${nearbySettlement.reward} oro`
                            : nearest
      ...
      return render.compose({
        ...
        footer: snap.combat
          ...
          : snap.mode === 'dungeon'
            ...
            : snap.mode === 'barbarian-camp'
              ? gameplayFooter(
                  autosave + 'wasd mover | toca barbaro | U salir | limpia todo para cobrar'
                )
              : snap.mode === 'boss'
                ? gameplayFooter(
                ...
                : snap.boss && snap.boss.active && !snap.boss.defeated
                  ...
                  : gameplayFooter(
                      autosave +
                        't volver a la ciudad | e hablar | < RUNA | N NOX | X cripta | O portal'
                    )
      })
    }
