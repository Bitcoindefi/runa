  ...
  view() {
    ...
    if (this.field) {
      ...
      return render.compose({
        ...
        stats: base.stats,
        log: base.log,
        // `<` no es una tecla, es el porton dibujado en el borde oeste del campo.
        // El pie lo nombraba como si se pudiera apretar, asi que la gente lo
        // apretaba, no pasaba nada, y se quedaba encerrada en la pradera. Es el
        // mismo error que decia que `s` abria el script.
        footer: 'flechas mover | camina al < del oeste para volver | r recargar | q salir'
      })
    }
