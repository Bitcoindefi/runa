...
class Runa {
  ...
  finishDuel(reason = 'termino') {
    ...
    if (result) this.say(`duelo terminado: gana ${this.duelName(result.winner)}`)
  ...
  onKey(msg) {
    ...
    if (this.duel && this.duel.active) {
      ...
      if (key.matches(msg, 'space', 'enter', 'f')) {
        this.duelInput(this.duel.self, { attack: true })
        return null
      }
    }
  ...
  view() {
    ...
    return render.mapScreen({
      ...
      footer: duel
        ...
        : autosave + 'wasd o flechas | puertas automaticas | e hablar / interactuar | q salir',
      map: {
