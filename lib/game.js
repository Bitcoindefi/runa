...
class Runa {
  ...
  onKey(msg) {
    ...
    if (this.title) {
      ...
      if (key.matches(msg, 'enter', 'space', 'n')) {
        this.naming = true
        this.nameError = ''
  ...
  move(dx, dy) {
    ...
    const map = MAPS[this.walker.mapId]
    const gate = map && map.fieldGate
    const nx = this.walker.x + dx
    const ny = this.walker.y + dy
    if (gate && nx >= gate.x1 && nx <= gate.x2 && ny >= gate.y1 && ny <= gate.y2) {
      this.field = new Field({ player: this.player })
      this.field.setScript(this.scriptSource)
      this.say('atravesas el porton y salis a la pradera. pulsa t para volver')
      this.announce()
      return
    }
  ...
  view() {
    ...
    if (this.field) {
      ...
      return render.compose({
        ...
        log: base.log,
        footer: snap.combat
          ? 'f / espacio atacar | wasd contra la hitbox atacar | cada tecla = un intercambio'
          : 'wasd mover | toca la hitbox del monstruo | f atacar | t volver a la ciudad'
      })
    }
