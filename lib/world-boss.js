 ...
 */
function makeFieldFrame(pose = 'idle', phase = 'despertar') {
  ...
  const fractured = phase === 'fractura' || phase === 'furia'
  const exposed = phase === 'furia'
  centre(0, exposed || pulsing ? '___/^^*^^\\___' : '___/^^R^^\\___')
  centre(1, fractured ? ".-'./_/___\\_\\.'-." : ".-'../_____\\..'-.")
  centre(
    2,
    exposed
      ? '/___/|.[*].[*].|\\___\\'
  ...
  centre(5, fractured ? '\\__/_|_\\_/_/_|_\\__/' : '\\____|_\\___/_|____/')
  centre(
    6,
    exposed ? '[|...\\.<***>./...|]' : pulsing ? '[|.....<.*.>.....|]' : '[|.....<.R.>.....|]'
  )
  ...
  centre(8, exposed ? '/|=====/=*\\=====|\\' : '/|===============|\\')
  centre(9, fractured ? '/.|..\\../|.|\\../..|.\\' : '/.|...../|.|\\.....|.\\')
  centre(10, fractured ? '__/..|.\\../_|.|_\\../.|..\\__' : '__/..|..../_|.|_\\....|..\\__')
  centre(11, exposed ? '/___/|._/_/*.*\\.\\_\\.|\\___\\' : '/___/|.._/./...\\.\\_..|\\___\\')
  centre(12, '/____/.|_/_/.....\\_\\_|.\\____\\')

  return canvas.map((row) => row.join(''))
}

const POSES = ['idle', 'idlePulse', 'punchLeft', 'punchRight', 'sweep', 'slam', 'slamImpact']
const PHASE_FRAMES = {}
for (const phase of ['despertar', 'fractura', 'furia']) {
  PHASE_FRAMES[phase] = {}
  for (const pose of POSES) PHASE_FRAMES[phase][pose] = makeFieldFrame(pose, phase)
}

/** Backwards-compatible base frames for consumers that do not know phases. */
const FIELD_FRAMES = PHASE_FRAMES.despertar
