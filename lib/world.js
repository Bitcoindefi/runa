...
class Actor {
   ...
   * @param {object[]} held
   * @returns {{ atk: number, reach: number, speed: number, cooldown: number }}
   */
  derive(held) {
...
class World {
  ...
  snapshot() {
    ...
    const held = [this.held.left, this.held.right]
    ...
    for (const a of actions) {
      switch (a.cmd) {
        ...
        case 'equipl': {
          ...
          if (!item) { problems.push(`no existe el item "${a.args[0] || ''}"`); break }
          if (a.cmd === 'equipl' || item.hand !== 'right') this.wanted.left = item
  ...
  step() {
    ...
    if (this.foe.alive) {
      ...
      if (dist > fd.reach) {
        ...
        this.say(`${this.foe.name} te pega ${fd.atk}`)
      }
    }
